...
At publish time, the repository root is also the Codex skill root. That means the installed skill can reference local files with stable relative paths.
...
`scripts/install_skill.sh` copies the repository into `${CODEX_HOME:-$HOME/.codex}/skills/evolution-constraint-planner` so the repo remains directly usable as a Codex skill.
