...
Evolution Constraint Planner, or ECL, is an original delivery method composed for Codex-based work. It is not a direct clone of a single paper, framework, or prompt chain. It is a practical synthesis built around one hard requirement:
...
ECL's theory comes from combining several engineering ideas into one operational loop.
...
ECL treats delivery like a compilation pipeline instead of an open-ended conversation. A raw request is compiled into a normalized case, then into a staged bundle, then into a code handoff, then into run evidence, and finally into an achieve verdict.
...
ECL assumes any plan written by one model is vulnerable to anchoring, omission, and optimism bias. It therefore forces independent criticism before execution.
...
## Why ECL Uses These Sources Together
...
ECL combines them so each stage limits a different failure mode.

## The Core ECL Thesis

The main thesis of ECL is:
...
## Why The Skill Is Codex-first

This repository binds the method to Codex because Codex gives ECL the concrete primitives it needs:

- local file access
- script execution
- staged artifact editing
- real spawned subagents

Without those primitives, ECL can be described, but not fully enacted.
