...
Evolution Constraint Planner，简称 ECL，是一套围绕 Codex 工作方式构建出来的原创交付方法。它不是从某一篇论文、某一个框架或者某一条 prompt chain 直接照搬出来的，而是围绕一个硬约束拼出来的：
...
ECL 把交付看作一个编译链，而不是一场开放式长对话。原始请求会先被编译成 normalized case，再被编译成 staged bundle，再被编译成 code handoff，然后是 run evidence，最后才进入 achieve verdict。
...
ECL 默认认为：任何由单个模型写出来的规划都容易被锚定、遗漏和乐观偏差污染。因此，在执行前必须插入独立视角的攻击和审查。

这直接体现在：

- D 阶段独立 critique
- G 阶段 red-team 与 blue-team
- H 阶段独立 coding-readiness review
- J 阶段独立 compile-for-code 判断
...
ECL 把它们组合起来，是为了让每个阶段卡住一种不同的失真来源。
...
## 为什么这个 Skill 明确是 Codex-first

这个仓库把方法和 Codex 绑在一起，是因为 Codex 恰好提供了 ECL 需要的几项关键能力：
